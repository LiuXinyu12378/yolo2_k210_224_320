...
class MobileNetFeature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size, weights=False):
        input_image = Input(shape=(224, 320, 3))
        mobilenet = MobileNet(input_shape=(224,320,3),alpha = 0.75,depth_multiplier = 1, dropout = 0.001, weights = 'imagenet', classes = 1000, include_top=False,backend=keras.backend, layers=keras.layers,models=keras.models,utils=keras.utils)
