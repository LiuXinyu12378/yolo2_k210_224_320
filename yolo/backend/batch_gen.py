...
def create_batch_generator(annotations, 
    ...
    img_aug = ImgAugment(224, 320, jitter)
...
class _YoloBox(object):
    ...
    def trans(self, boxes):
        ...
        centroid_boxes = to_centroid(boxes).astype(np.float32)     #中心点xy   wh
        # print("centroid_boxes:", centroid_boxes)
        # 2. image scale -> grid scale
        radio_h = self._grid_size[0]/self._input_size[0]
        radio_w = self._grid_size[1]/self._input_size[1]
        norm_boxes = []
        # norm_boxes = centroid_boxes * (self._grid_size / self._input_size)
        for centroid_boxe in centroid_boxes:
            centroid_boxe[0] = centroid_boxe[0] * radio_h
            centroid_boxe[1] = centroid_boxe[1] * radio_w
            centroid_boxe[2] = centroid_boxe[2] * radio_h
            centroid_boxe[3] = centroid_boxe[3] * radio_w
            norm_boxes.append(centroid_boxe)
